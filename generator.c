...
struct testcase * generator_radamsa(char * count, char * testcase_dir, char * path, char * prefix, int seed){
    pid_t pid;
    int s;
    char output[PATH_MAX];
    struct testcase * testcase;
    char mySeed[10] = {0} ;
    snprintf (mySeed, 10, "%d", seed) ;

    snprintf(output, PATH_MAX, "%s/%s-%%n", path, prefix);
    char * argv[] = { "radamsa", "-s", mySeed, "-n", count, "-r", "-o", output, testcase_dir, 0 };
