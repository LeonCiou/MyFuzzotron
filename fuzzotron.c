...
int main(int argc, char** argv) {
    ...
    int arg_index;
    while((c = getopt_long(argc, argv, "d:c:h:p:g:t:m:c:P:r:w:s:z:o:S:", arg_options, &arg_index)) != -1){
        switch(c){
            case 'c':
            ...
            case 'P':
                // define protocol
                if((strcmp(optarg,"udp") != 0) && (strcmp(optarg,"tcp") != 0) && (strcmp(optarg,"unix") != 0) && (strcmp(optarg, "sctp") != 0)){
                        fatal("Please specify either 'tcp', 'udp', 'sctp' or 'unix' for -P\n");
                }
...
void * worker(void * worker_args){
    ...
    while(1){
        ...
        if(fuzz.gen == BLAB){
            ...
            cases = generator_radamsa(CASE_COUNT, fuzz.in_dir, fuzz.tmp_dir, prefix, seed);
		}
...
void help(){
    ...
    printf ("\t--seed\t\tSet a seed for radamsa.\n\n") ;
