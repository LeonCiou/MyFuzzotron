 ...
 * sctp socket
 */
int send_sctp(char* host, int port, char* packet, unsigned long packet_len){
    ...
    //servaddr.sin_addr.s_addr = inet_addr ("127.0.0.1") ;    /* 因為這些函式無法處理 IPv6，所以建議不要使用！請使用 inet_ntop() 或 inet_pton() 來代替！ */
    ...
    fcntl(connSock, F_SETFL, O_RDONLY|O_NONBLOCK) ;
    // ret = write(connSock, packet, packet_len) ;
    ret = sctp_sendmsg(connSock, (void *) packet, (size_t) packet_len, NULL, 0, 0, 0, 0, 0, 0) ;
    //ret = sctp_sendmsg(connSock, (void *) "BB", (size_t) 2, NULL, 0, 0, 0, 0, 0, 0) ;
